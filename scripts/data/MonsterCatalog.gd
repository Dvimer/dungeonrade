...
static func roll_monster(rng: RandomNumberGenerator) -> MonsterType:
	var total := 0
	for data in DEFINITIONS.values():
		total += int(data.get("spawn_weight", 1))
	var roll := rng.randi_range(1, max(1, total))
	var acc := 0
	for data in DEFINITIONS.values():
		acc += int(data.get("spawn_weight", 1))
		if roll <= acc:
			return MonsterType.new(data)
	return get_monster(DEFAULT_MONSTER_ID)

static func make_enemy_tile(rng: RandomNumberGenerator, monster_id: String = "") -> Dictionary:
	var monster := get_monster(monster_id) if monster_id != "" else roll_monster(rng)
